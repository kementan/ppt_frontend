...
export class UserListComponent implements OnInit {
  ...
  public sec1_title: string = "Data User";
  public data: any[] = [];
  public filter: any = {
    search: "",
    sortBy: "id",
  ...
  constructor(private userService: UserService, public loadingService: LoadingService) { }
  ...
  ngOnInit(): void {
    ...
    this.userService.visibleChanged.subscribe((value) => {
      this.visible = value;
    });
  ...
  exportData() {
    this.userService.exportData().subscribe((response: Blob) => {
  ...
  doCreate(): void {
    ...
    this.userService.setLabel("Data Baru");
    this.userService.toggleVisible();
  }

  doEdit(id: string) {
    this.resetIDs();
    this.userService.setLabel("Edit Data");
  ...
  clearSession(): void {
    Swal.fire({
      title: "Clear Sesi Redis ?",
      text: "User akan dapat login pada beda device meski belum 1 jam",
      icon: "question",
      ...
      if (result.isConfirmed) {
        this.userService.removeSession().subscribe({
          next: res => {
              if (res.status == 200) {
                msg.fire(
                  "Berhasil", 
  ...
  doDelete(): void {
    if (this.selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.loadingService.showLoading();
          this.userService.deleteData(this.selectedIds).subscribe({
            next: () => {
              this.selectedIds = [];
