...
export class RoleListComponent {
  ...
  public sec1_title: string = "Data Role";
  public data: any[] = [];
  public filter: any = {
    search: "",
    sortBy: "id",
  ...
  constructor(private roleService: RoleService, public loadingService: LoadingService) { }
  ...
  ngOnInit(): void {
    ...
    this.roleService.visibleChanged.subscribe((value) => {
      this.visible = value;
    });
    ...
    this.roleService.loadData$.subscribe(() => {
      this.loadData();
    });
  ...
  exportData() {
    this.roleService.exportData().subscribe((response: Blob) => {
  ...
  doCreate(): void {
    this.resetIDs();
    this.roleService.setID(null);
    this.roleService.setLabel("Data Baru");
    this.roleService.toggleVisible();
  }

  doEdit(id: string) {
    this.resetIDs();
    this.roleService.setLabel("Edit Data");
    ...
    if (this.selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.loadingService.showLoading();
          this.roleService.deleteData(this.selectedIds).subscribe({
            next: () => {
              this.selectedIds = [];
