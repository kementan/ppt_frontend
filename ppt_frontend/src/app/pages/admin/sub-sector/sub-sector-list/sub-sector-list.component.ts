...
@Component({
  ...
  styleUrls: ['./sub-sector-list.component.scss']
})
export class SubSectorListComponent {
  private searchTerms = new Subject<string>();
  private totalRecords: number = 0;
  private sortedColumnIndex: number | null = null;
  private selectedIds: string[] = []; 
  private selectedNames: string[] = []; 
  public sec1_title: string = "Data Sub Sektor";
  public data: any[] = [];
  public filter: any = {
    search: "",
    sortBy: "id",
  ...
  constructor(private commodityService: SubSectorService, public loadingService: LoadingService) { }
  ...
  ngOnInit(): void {
    ...
    this.commodityService.visibleChanged.subscribe((value) => {
      this.visible = value;
    });
    ...
    this.commodityService.loadData$.subscribe(() => {
      this.loadData();
    });
  ...
  loadData(): void {
    ...
    this.commodityService.getList(this.filter).subscribe({
      next: res => {
  ...
  exportData() {
    this.commodityService.exportData().subscribe((response: Blob) => {
  ...
  doCreate(): void {
    this.resetIDs();
    this.commodityService.setID(null);
    this.commodityService.setLabel("Data Baru");
    this.commodityService.toggleVisible();
  }

  doEdit(id: string) {
    this.resetIDs();
    this.commodityService.setLabel("Edit Data");
    this.commodityService.setID(id);
    this.commodityService.toggleVisible();
  }
    ...
    if (this.selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.loadingService.showLoading();
          this.commodityService.deleteData(this.selectedIds).subscribe({
            next: () => {
              this.selectedIds = [];
