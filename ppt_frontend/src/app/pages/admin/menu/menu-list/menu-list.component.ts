...
export class MenuListComponent implements OnInit {
  ...
  public sec1_title: string = "Data Menu";
  public data: any[] = [];
  public filter: any = {
    search: "",
    sortBy: "sort",
  ...
  constructor(private menuService: MenuService, public loadingService: LoadingService) { }
  ...
  ngOnInit(): void {
    ...
    this.menuService.visibleChanged.subscribe((value) => {
      this.visible = value;
    });
  ...
  loadData(): void {
    ...
    this.menuService.getList(this.filter).subscribe({
      next: res => {
  ...
  exportData() {
    this.menuService.exportData().subscribe((response: Blob) => {
  ...
  doCreate(): void {
    this.resetIDs();
    this.menuService.setID(null);
    this.menuService.setLabel("Data Baru");
    this.menuService.toggleVisible();
  }

  doEdit(id: string) {
    this.resetIDs();
    this.menuService.setLabel("Edit Data");
    this.menuService.setID(id);
    this.menuService.toggleVisible();
  }
    ...
    if (this.selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.loadingService.showLoading();
          this.menuService.deleteData(this.selectedIds).subscribe({
            next: () => {
              this.selectedIds = [];
