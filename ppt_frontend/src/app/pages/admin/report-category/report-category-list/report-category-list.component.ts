...
export class ReportCategoryListComponent {
  ...
  constructor(private reportCategoryService: ReportCategoryService, public loadingService: LoadingService) { }
  ...
  ngOnInit(): void {
    ...
    this.reportCategoryService.visibleChanged.subscribe((value) => {
      this.visible = value;
    });
    ...
    this.reportCategoryService.loadData$.subscribe(() => {
      this.loadData();
    });
  ...
  loadData(): void {
    ...
    this.reportCategoryService.getList(this.filter).subscribe({
      next: res => {
  ...
  exportData() {
    this.reportCategoryService.exportData().subscribe((response: Blob) => {
  ...
  doCreate(): void {
    this.resetIDs();
    this.reportCategoryService.setID("");
    this.reportCategoryService.setLabel("Data Baru");
    this.reportCategoryService.toggleVisible();
  }

  doEdit(id: string) {
    this.resetIDs();
    this.reportCategoryService.setLabel("Edit Data");
    this.reportCategoryService.setID(id);
    this.reportCategoryService.toggleVisible();
  }
    ...
    if (this.selectedIds.length > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.loadingService.showLoading();
          this.reportCategoryService.deleteData(this.selectedIds).subscribe({
            next: () => {
              this.selectedIds = [];
