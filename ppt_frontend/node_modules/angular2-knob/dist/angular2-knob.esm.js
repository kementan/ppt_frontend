...
import { arc, drag, easeBounce, easeLinear, event, interpolate, mouse, range, select } from 'd3';
...
var Ng2KnobDirective = /** @class */ (function () {
    ...
    Ng2KnobDirective.prototype.drawArcs = function (clickInteraction, dragBehavior) {
        ...
        if (this.options.scale.enabled) {
            ...
            if (this.options.scale.type === 'dots') {
                ...
                data = range(quantity_1).map(function () {
    ...
    Ng2KnobDirective.prototype.draw = function () {
        ...
        if (that.options.animate.enabled) {
            ...
            that.valueElem.transition().ease(that.animations[that.options.animate.ease]).duration(that.options.animate.duration).tween('', function () {
                var i = interpolate(that.valueToRadians(that.options.startAngle, 360), that.valueToRadians(that.value, that.options.max, that.options.endAngle, that.options.startAngle, that.options.min));
                return function (t) {
                    var val = i(t);
        ...
        function dragInteraction() {
            that.inDrag = true;
            var x = event.x - (that.options.size / 2);
            var y = event.y - (that.options.size / 2);
            interaction(x, y, false);
        }
        function clickInteraction() {
            that.inDrag = false;
            var coords = mouse(this.parentNode);
        ...
        function interaction(x, y, isFinal) {
            ...
            if (that.value >= that.options.min && that.value <= that.options.max) {
                ...
                if (that.options.displayInput) {
                    ...
                    select(that.element).select('#text').text(v + that.options.unit || '');
                }
            }
    ...
    Ng2KnobDirective.prototype.setValue = function (newValue) {
        if ((!this.inDrag) && this.value >= this.options.min && this.value <= this.options.max) {
            ...
            select(this.element).select('#changeArc').attr('d', this.changeArc);
            this.valueArc.endAngle(radians);
            select(this.element).select('#valueArc').attr('d', this.valueArc);
            if (this.options.displayInput) {
                var v = this.label || this.value;
                if (typeof this.options.inputFormatter === 'function') {
                    v = this.options.inputFormatter(v);
                }
                select(this.element).select('#text').text(v + this.options.unit || '');
            }
        }
    };
    __decorate([
...
export { KnobModule };
//# sourceMappingURL=angular2-knob.esm.js.map
